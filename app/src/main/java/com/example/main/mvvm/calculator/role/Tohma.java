...
public class Tohma extends Obj_calculator{
    public Tohma(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public List<Sh_data> csh() {
        ...
        g2();
        data.add(new Sh_data("E护盾上限",new double[]{(getSmz(0)*getbl(e_level,1,1,3)+getbl(e_level,1,3,3))*getHD(),0}));
        return data;
    }
    ...
    public void g2() {
        if (dj>=70)
            if (!addZy(new Zy(0.25, "天赋2", true, false, "81", 0, 0)))
