...
public class Kazuha extends Obj_calculator{
    public Kazuha(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public List<Sh_data> csh() {
        ...
        data.add(new Sh_data("元素伤害加成",new double[]{getYsjt(1)*0.04,0}));
        return data;
    }
        ...
        switch (mz){
            ...
            case 3:
                if (e_level < 11)
                    e_level += 3;
            case 2:
                if (!addZy(new Zy(200, "命座2_", false, false, "28", getTime(), 0))) {
