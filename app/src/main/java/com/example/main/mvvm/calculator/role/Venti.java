...
public class Venti extends Obj_calculator {
    public Venti(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public List<Sh_data> csh() {
        ...
        data.add(new Sh_data("点按E", user_e()));
    ...
    public double[] user_q() {
        ...
        sh_ysh += (getGj(0) * getbl(q_level, 2, 1, 1) + getJc("30045", "10023")) * getZs("45", "10003") * getFy() * getYsKx("45");
    ...
    public void user_mz(int mz) {
        switch (mz) {
            case 6:
                if (!addZy(new Zy(0.2, "命座6", false, false, "20044", getTime(), 0))) {
                    accumulateValue("20044", 0.2);
                }
            case 5:
                if (e_level < 11)
                    e_level += 3;
            ...
            case 3:
                if (q_level < 11)
                    q_level += 3;
            case 2:
                if (!addZy(new Zy(0.24, "命座2", false, false, "20044", getTime(), 0))) {
