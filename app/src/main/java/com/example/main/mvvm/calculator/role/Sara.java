...
public class Sara extends Obj_calculator{
    public Sara(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public List<Sh_data> csh() {
        ...
        g2();
        g3();
        double zs=jszsg_fb.getFightPropMap().get("4")*getbl(e_level,1,1,1);
        data.add(new Sh_data("攻击力提升",new double[]{zs,0}));
        if (!addZy(new Zy(zs, "九条e增伤", false, false, "5", 0, 0))) {
            accumulateValue("5", zs);
        }
        data.add(new Sh_data("吃E后Q首段",user_q()));
        return data;
    }
    ...
    public double[] user_q() {
        double[] d;
        double sh_ysh = (getGj(0) * (getbl(q_level, 2, 1, 0)) + getJc("30041","10024")) * getZs("41", "10003") * getFy() * getYsKx("41");
