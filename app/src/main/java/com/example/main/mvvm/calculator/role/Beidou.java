...
public class Beidou extends Obj_calculator {
    public Beidou(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public double[] user_q() {
        double[] d;
        double sh_ysh = (getGj(0) * ( getbl(q_level, 2, 1, 1)) + getJc("30041","10024")) * getZs("41", "10003") * getFy() * getYsKx("41");
    ...
    public double[] user_e() {
        double[] d;
        double sh_ysh = (getGj(0) * ( getbl(e_level, 1, 1, 1)) + getJc("30041","10023")) * getZs("41", "10002") * getFy() * getYsKx("41");
    ...
    public void user_mz(int mz) {
        switch (mz) {
            case 6:
                if (!addZy(new Zy(0.15, "命座六", false, false, "20041", getTime(), 8))) {
                    accumulateValue("20041", 0.15);
                }
            case 5:
                if (q_level < 11)
                    q_level += 3;
