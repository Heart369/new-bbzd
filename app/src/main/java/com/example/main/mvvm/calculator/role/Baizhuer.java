...
public class Baizhuer extends Obj_calculator {
    public Baizhuer(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public List<Sh_data> csh() {
        ...
        Log.d("TAGZS", getCure() + "," + (getSmz(0) * getbl(q_level, 2, 1, 2) + getbl(q_level, 2, 3, 2)));
        data.add(new Sh_data("E治疗量", new double[]{(getSmz(0) * getbl(e_level, 1, 1, 1) + getbl(e_level, 1, 3, 1)) * getCure() * a, 0}));
        data.add(new Sh_data("Q治疗量", new double[]{(getSmz(0) * getbl(q_level, 2, 1, 2) + getbl(q_level, 2, 3, 2)) * getCure(), 0}));
        data.add(new Sh_data("Q护盾", new double[]{(getSmz(0) * getbl(q_level, 2, 1, 0) + getbl(q_level, 2, 3, 0)) * getHD(), 0}));
