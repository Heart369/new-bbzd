...
public class Razor extends Obj_calculator{
    public Razor(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public List<Sh_data> csh() {
        ...
        data.add(new Sh_data("平A第一段伤害",user_a(0)));
        return data;
    }
    ...
    public double[] user_a(int a) {
        double[] d;
        double sh_ysh = 0;
        sh_ysh = (getGj(0) * getbl(a_level, 0, 0, 0) + getJc("30030","10025")) * getZs("30", "10001") * getFy() * getYsKx("30");
        d = getqw("10030", sh_ysh, 0);
        return d;
    }
        ...
        switch (mz) {
            ...
            case 5:
                if (e_level < 11)
                    e_level += 3;
            case 4:
                if (!addZy(new Zy(0.15, "4命", false, false, "20030", 0, 0))) {
