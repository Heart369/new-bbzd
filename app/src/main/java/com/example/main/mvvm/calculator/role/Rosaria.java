...
public class Rosaria extends Obj_calculator {
    public Rosaria(Jszsg.AvatarInfoListDTO jszsg, List<Bl_cl> bl_cl, CharacterData du, Context context) {
    ...
    public List<Sh_data> csh() {
        ...
        data.add(new Sh_data("E两段伤害", user_e()));
        data.add(new Sh_data("Q持续伤害", user_q()));
        return data;
    }
    ...
    public double[] user_q() {
        double[] d;
        double sh_ysh = (getGj(0) * (getbl(q_level, 2, 1, 1))+ getJc("30030","10024")) * getZs("46", "10003") * getFy() * getYsKx("46");
    ...
    public double[] user_e() {
        double[] d;
        double sh_ysh = (getGj(0) * ((getbl(e_level, 1, 1, 0)+getbl(e_level,1,3,0)))+ getJc("30030","10023")) * getZs("46", "10002") * getFy() * getYsKx("46");
        d = getqw("100046", sh_ysh, 2);
        return d;
    }
